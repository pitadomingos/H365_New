...
export function AIAssistantPanel({ context, department, patientData, onAcceptSuggestion }: AIAssistantPanelProps) {
  ...
  const handleAIRequest = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `${clinicalContext}\n\nClinical Inquiry: ${prompt || "Analyze the current patient data for potential anomalies or treatment optimizations."}`,
      });

      const resultText = response.response.text();
      setResult(resultText || "No insights generated.");
    } catch (err: any) {
      console.error("AI Assistant Error:", err);
      // Fallback to queue if network fails
      setError("Limited connectivity detected. Requesting deferred audit instead...");
      setTimeout(() => {
        handleQueueRequest();
      }, 2000);
    } finally {
  ...
  const handleQueueRequest = () => {
    addToQueue({
      ...
      department: department || "General",
      prompt: prompt || "Full Case Audit",
      context: patientData,
      patientName: patientData?.fullName,
      patientId: patientData?.id
    });
    setPendingCount(prev => prev + 1);
    setResult("Record successfully queued for deferred Clinical AI Audit. Insights will be available after the next system sync (Broadband window).");
    setPrompt("");
    setError(null);
  };
    ...
    <Card className={cn("border-primary/20 shadow-sm transition-all duration-300", isOfflineMode ? "bg-orange-50/50" : "bg-primary/5")}>
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-xs">
          {isOfflineMode 
            ? "Limited connectivity. AI audits will be batch-processed during sync windows."
            : "Real-time clinical reasoning and data synthesis support."}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Textarea 
            placeholder={isOfflineMode ? "Input clinical notes for deferred AI quality audit..." : "Ask AI for differential diagnosis, medication review, or summary..."} 
          ...
          <div className="flex justify-between items-center">
            ...
            >
              ...
              {isAnalyzing ? "Analyzing..." : (isOfflineMode ? "Queue for Audit" : "Query AI Assistant")}
            </Button>
          </div>
        </div>
        ...
        {showQueue && (
           <div className="border rounded-md bg-background/80 max-h-[200px] overflow-y-auto p-2 space-y-2">
              <div className="flex justify-between items-center px-1 mb-2">
                <h4 className="text-[10px] font-bold uppercase text-muted-foreground">Audit & Sync History</h4>
                {pendingCount > 0 && !isOfflineMode && (
                  <Button variant="link" size="sm" className="h-auto p-0 text-[10px]" onClick={handleSync}>Sync Now</Button>
                )}
              </div>
              {getAIQueue().map((item) => (
                <div key={item.id} className="text-[10px] p-2 border rounded bg-background flex justify-between items-start">
                   <div className="space-y-1">
                      <p className="font-semibold">{item.patientName || "Anonymous Patient"}</p>
                      <p className="text-muted-foreground line-clamp-1">{item.prompt}</p>
                      {item.result && <p className="text-primary italic mt-1 bg-primary/5 p-1 rounded">Insight Available</p>}
                   </div>
              ...
              {getAIQueue().length === 0 && <p className="text-center text-[10px] py-4 text-muted-foreground">No pending audits.</p>}
           </div>
        )}
        ...
        {result && (
          <div className="space-y-3">
            <div className="p-4 bg-background border rounded-md shadow-sm relative group overflow-hidden">
               <div className={cn("absolute top-0 left-0 w-1 h-full", isOfflineMode && result.includes("queued") ? "bg-orange-500" : "bg-primary")} />
               <div className="flex items-center justify-between mb-2">
                 <span className="text-[10px] font-bold text-muted-foreground uppercase flex items-center gap-1">
                   <ShieldCheck className="h-3 w-3" /> {isOfflineMode && result.includes("queued") ? "Queue Confirmation" : "Verifiable Suggestion"}
                 </span>
                 <span className="text-[10px] text-muted-foreground">{new Date().toLocaleTimeString()}</span>
               </div>
               <div className={cn("text-sm leading-relaxed whitespace-pre-wrap text-foreground", !result.includes("queued") && "italic")}>
                 {result}
               </div>

               {onAcceptSuggestion && !result.includes("queued") && (
               ...
               <div className="mt-4 pt-2 border-t border-dashed flex justify-between items-center opacity-70">
                 <p className="text-[10px] text-muted-foreground flex items-center gap-1 italic">
                    <AlertCircle className="h-3 w-3" /> clinician review required before action
                 </p>
      ...
      <CardFooter className="pt-0 pb-4 justify-center">
        <p className="text-[9px] text-muted-foreground uppercase font-medium">
          Audit Trial: Every AI interaction is logged with clinician ID and timestamp
        </p>
      </CardFooter>
    </Card>
  );
